...
export class AppComponent  {
  ...
  reestimate() {
    const estimatedValue = 1 + (this.maxOutcome - 1) * (this.complexity + this.knowledge + this.dependencies + this.communication - 4) / 8;
    this.estimation = this.findNearestStoryPoint(estimatedValue);
  }
  findNearestStoryPoint(goal:number):number{
    return this.storypoints.reduce((prev, curr) => Math.abs(curr - goal) < Math.abs(prev - goal) ? curr : prev);
  }
}
